class Form{
    constructor(){
       ...
       this.instruction.style("color","darkblue") ;
       this.button1 = createButton("Next");
       this.button2 = createButton("h");
       this.button2.position(300, this.button1.y);
       this.button3 = createButton("m");
       this.button2.position(270, this.button2.y);
    ...
    gameStateZero(){
        background(bg_img);
            this.instruction.position(windowWidth-500, 60);
                if(gameState == 0 && touches.length > 1 ){
    ...
    gameStateOne(){
        this.instruction.html("Congrats You Have Taken<br> The Measures To Protect<br> Yourself.Double Tap the screen  to go<br> to the next level");
            if(gameState == 1 && touches.length > 1){
                gameState = 2;
                console.log("yup");
                touches = [];
            }
    ...
    gameStateFour(){
        ...
        //player.debug = true;
            ...
            if(gameState == 4 &&keyDown(UP_ARROW)  || touches [touches.length-1].y < player.y ){
                y2 = y2-100;
            ...
            if(gameState == 4 && keyDown(LEFT_ARROW) || touches[touches.length - 1].x < player.x){
                x2 = x2-20;
                //console.log("it works : " + y2);
            ...
            if(gameState == 4 && keyDown(RIGHT_ARROW) || touches[touches.length - 1].x > player.x){
                x2 = x2+20;
                //console.log("it works : " + y2);
